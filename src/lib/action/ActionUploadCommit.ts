...
import {ReqUploadCommit, File, Size} from "lib/action/req/ReqUploadCommit";
...
import {RespUploadCommit} from "lib/action/resp/RespUploadCommit";
import {FileUploaded} from "lib/file/FileUploaded";
import {FileCommited} from "lib/file/FileCommited";
...
export class ActionUploadCommit extends AActionUploadId {
    ...
    public run(request: Req, onFinish: (result: RespOk) => void): void {
        let req: ReqUploadCommit = request as ReqUploadCommit;
            ...
            (e: Error) => {
                ...
                if (e != null) {
                    if (e instanceof MessageException)
                        onFinish(new RespFail((e as MessageException).getFailMessage()));
                    else
    ...
    private filesCommited: FileCommited[];
    private filesCommitedCount: number;
    private applySizes(
